...
def addEURPallet() -> None:
    ...
    positions = [(0+0.145/2, 0+0.1/2, 0.022), (0+0.145/2, 0.4, 0.022), (0+0.145/2, 0.8-0.1/2, 0.022)]
    sizes = [(0.145, 0.1, 0.078), (0.145, 0.145, 0.078), (0.145, 0.1, 0.078)] 
    for pos, size in zip(positions, sizes):
        __addPalObject(pos, size)
        
    positions = [(1.2-0.145/2, 0+0.1/2, 0.022), (1.2-0.145/2, 0.4, 0.022), (1.2-0.145/2, 0.8-0.1/2, 0.022)]
    sizes = [(0.145, 0.1, 0.078), (0.145, 0.145, 0.078), (0.145, 0.1, 0.078)] 
    for pos, size in zip(positions, sizes):
        __addPalObject(pos, size)
...
def __addRCObject(position:list, size:list) -> None:
    ...
    translatedPosition = position[0]+offsetX, position[1]+offsetY, position[2]+size[2]/2+offsetZ
    bpy.ops.mesh.primitive_cube_add(size=1, enter_editmode=False, align='WORLD', location=list(translatedPosition), scale=size)
...
def __initScene(target:str) -> None:
    ...
    # init the camera
    if target=="rollcontainer": camPosition = (2.3388, -2.3318, 2.1809)
    elif target=="euro-pallet": camPosition = (2.6788, -2.4603, 2.1692)
    bpy.ops.object.camera_add(enter_editmode=False, align='VIEW', location=camPosition, rotation=(1.22872, -4.53789e-07, 0.642278), scale=(1, 1, 1))
    bpy.context.object.name="Camera"
    bpy.context.scene.camera = bpy.context.object
    
    __addGround(target=target)
    if target=="rollcontainer": addRollcontainer()
    elif target =="euro-pallet": addEURPallet()
...
if __name__ == '__main__':
    ...
    for i, action in enumerate(ORDER_PP):
        ...
        if orientation == 0:
            size = (item["length/mm"]/SCALE_DIVISOR, item["width/mm"]/SCALE_DIVISOR, item["height/mm"]/SCALE_DIVISOR)
        elif orientation == 1:
            size = (item["width/mm"]/SCALE_DIVISOR, item["length/mm"]/SCALE_DIVISOR, item["height/mm"]/SCALE_DIVISOR)
    ...
    movementValues = {
        "mean_z-movements/m": statistics.fmean(zMovements),
        "max_z-movements/m": max(zMovements)
    }
